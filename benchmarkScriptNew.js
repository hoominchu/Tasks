            ...
            chrome.storage.local.get("Text Log", function (textLog) {
                if (textLog["Text Log"]) {
                    for (var taskId in TASKS) {
                        if (taskId != "lastAssignedId" && taskId != "0") { //pick up taskId
                            for (var index = 0; index < TASKS[taskId].tabs.length; index++) { // pick up tab

                                var urlToCheck = TASKS[taskId].tabs[index].url;  // Looking at suggestions for this url
                                try {
                                    var modifiedTasks = createTasksWithoutTab(taskId, index);
                                    var contentString = pageContentObject["Page Content"][urlToCheck]; //content for urlToCheck
                                    var tagsForUrlToCheck = getNamedEntityTagsOnDocument(contentString); //tags for urlToCheck
                                    var suggestion = newTaskDetector(urlToCheck, modifiedTasks, textLog["Text Log"], tagsForUrlToCheck, taskId, SETTINGS);
...
function newTaskDetector(urlToCheck, modifiedTasks, textLog, tagsForUrlToCheck, actualTaskId, settings) {

    var taskURLs = getTaskURLs(modifiedTasks);

    var results = getTaskwiseTotalScores(tagsForUrlToCheck, textLog, taskURLs);
    ...
    return createSuggestion(urlToCheck, taskWiseTotalScoresArray, allMatchedTags, modifiedTasks, actualTaskId, settings)
...
function getNamedEntityTagsOnDocument(contentString) {
    var tags = {};
    ...
    for (var i = 0; i < topics.length; i++) {
        ...
        if (isValidTag(topic)) {
            ...
            for (var j = 0; j < tokens.length; j++) { // Add "Great" and "Wall" from "Great Wall"
                var token = tokens[j];
                if (isValidTag(token)) {
            ...
            if (topic.split(" ").length > 1) { // Add "Great Wall"
...
function getTaskwiseTotalScores(tags, tagLog, taskURLs) {
    ...
    for (var gh in taskURLs) {
        taskwiseTotalScores[gh] = 0;
    }
...
function returnConfidence(matchesWithTask1, matchesWithTask2) {
    var diff = matchesWithTask1 - matchesWithTask2;
    var confidence = diff / matchesWithTask1;
    return confidence;

}

function createSuggestion(url, taskWiseTotalScoresArray, allMatchedTags, tasks, actualTaskId, settings) {
    if (taskWiseTotalScoresArray.length > 1) {
        ...
        var confidence = returnConfidence(taskWiseTotalScoresArray[0][1], taskWiseTotalScoresArray[1][1]);
        ...
        var suggestion = new Suggestion(url, allMatchedTags[mostProbableTaskID], TASKS[actualTaskId].name, mostProbableTaskName, correctValue, confidence);
    }
...
function downloadMetricsOnSuggestions(threshold){
    var falsePositives = Suggestions.filter(function(sugg) { return (sugg.isCorrect == false)  && (sugg.confidence>=threshold)});
    var falseNegatives = Suggestions.filter(function(sugg) { return (sugg.isCorrect == true)  && (sugg.confidence<threshold)});
    var truePositives = Suggestions.filter(function(sugg) { return (sugg.isCorrect == true)  && (sugg.confidence>=threshold)});
    var trueNegatives = Suggestions.filter(function(sugg) { return (sugg.isCorrect == false)  && (sugg.confidence<threshold)});
    ...
    suggResults["false-positives"] = falsePositives;
    suggResults["false-negatives"] = falseNegatives;
    suggResults["true-positives"] = truePositives;
    suggResults["true-negatives"] = trueNegatives;

    saveJSON(suggResults, "Suggestions on " + " " + threshold);
