...
// chrome.storage.local.get("Text Log", function(textLogObject){
//     if(textLogObject["Text Log"]){
...
//                             try{
//                                 var suggestion = newTaskDetector(TASKS[taskId].tabs[index].url, contentString, createTasksWithoutTab(taskId, index), pageContentObject["Page Content"], taskId, SETTINGS);
...
chrome.storage.local.get("Page Content", function(pageContentObject) {
    var pageContent = pageContentObject["Page Content"];
    if (pageContent[url]) {
        for (var taskId in TASKS) {
            if (taskId != "lastAssignedId") {
                for (var index = 0; index < TASKS[taskId].tabs.length; index++) {
                    var url = TASKS[taskId].tabs[index].url;
                    var contentString = pageContentObject["Page Content"][TASKS[taskId].tabs[index].url];
                    try {
                        createTextLogWithoutTab(url, taskId, function (textLog) {
                            var suggestion = newTaskDetector(url, contentString, textLog, taskId, SETTINGS);
                            console.info(suggestion);
                            Suggestions.push(suggestion);
                        });
                        ...
                        console.error("Can't do it for %s", url);
                    }
                }
            }
        }
...
function getNamedEntityTagsOnCurrentDocument(contentString) {
    ...
    var contentString = cleanTag(contentString);
...
function newTaskDetector(url, contentString, textLog, tasks, actualTaskId, settings) {
...
function createTextLogWithoutTab(url, taskId, callback){
    var url = url;
    var tags = getNamedEntityTagsOnCurrentDocument(pageContent[url]);
    chrome.storage.local.get("Text Log", function(textLogObject){
        var textLog = textLogObject["Text Log"];
        for(var tag in tags){
            if(textLog.hasOwnProperty(tag)){
                delete textLog[tag]["tasks"][taskId][url];
            }
        }
        callback(textLog);
    });
