    ...
    for (var task_id in Tasks) {
        if (task_id != "lastAssignedId") {
            ...
            if (Tasks[task_id].isActive) {
                var card = $("<div>", {
                    "class": "card border-dark mb-3 task",
            ...
            var open_button = $("<button class='btn btn-outline-success btn-sm round-corner-left' type='button' id='" + Tasks[task_id].id + "'>Open</button>");
            var delete_button = $("<button class='btn btn-outline-danger btn-sm round-corner-left' type='button' id='" + Tasks[task_id].id + "'>Delete</button>");
            var rename_button = $("<button class='btn btn-outline-dark btn-sm round-corner' type='button' id='" + Tasks[task_id].id + "'>Rename</button>");
            var share_button = $("<button class='btn btn-outline-primary btn-sm round-corner-right' type='button' id='" + Tasks[task_id].id + "'>Share</button>");
            var archive_button = $("<button class='btn btn-outline-primary btn-sm round-corner-right' type='button' id='" + Tasks[task_id].id + "'>Archive</button>");
            card_buttons.append(open_button);
            card_buttons.append(rename_button);
            card_buttons.append(share_button);
